...
function EndSkin(tmpId)
{
	...
	{	
		...
		tempPath = file.replace(/\\[a-zA-Z_0-9]*\.jsp$/, "/"); 
		// console.log("+++++++++++++++++++++++++++++++++++++++" + file);
	...
	this.getRole = function(){
		...
		// var file = 'views/setRoleConfig.txt';
		// var fileMatch = file.match(/views\/(.*?)\w/i);
	...
	this.getTemplateString = function(id)
	{
		...
		var smatch;
		if(!role){
			role = this.getRole();//choice of role:guset,teacher,user	
		}
		...
		//remove taglib.jsp START
		if(smatch = s.match(/<%@.*?include.*file=\"(.*?)\/taglib.jsp\".*?%>/ig)){
			// console.log(smatch);
			for(i=0; m=smatch[i]; i++)
			{
				s = s.replace(m, '');
		...
		//<duobei:script />   
		var ms = s.match(/\<duobei\:script.*\/\>/ig);
		if(ms){
			for (var i = 0; m = ms[i]; i++) {
				var _ms = m.match(/\<duobei\:script.*src=\"(.*?)\".*?\/\>/);
				if(_ms){
					var src = "${duobei_javascript}/" + _ms[1],
						nodeScript = "<script type='text/javascript' src='" + src + "' charset='utf-8'></script>";
					s = s.replace(m, nodeScript);
				}
			}
		}
